...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body with SWV printing
  ******************************************************************************
  */
...
/* USER CODE BEGIN PV */
//how many time do I check the sensor? :
uint16_t delayValue = 200;
#define SINE_SAMPLES 256
uint16_t SineTable[SINE_SAMPLES];
uint16_t zoneThresholds[9] = {400, 800, 1200, 1600, 2000, 2400, 2800, 3000, 3200};
uint16_t prevZone = 1;
//beep request
volatile uint16_t beepRequest = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint32_t*)SineTable,
  ...
  while (1)
  {
      ...
      if (currTime - prevTime >= delayValue)
      {
      ...
      if (beepRequest > 0)
      {
          PlaySineBeep(beepRequest);
          beepRequest = 0;  // reset flag
      }
  }
  /* USER CODE END 3 */
}
...
void UpdateSineTable(uint16_t amplitude)
{
    ...
    for (int i = 0; i < SINE_SAMPLES; i++) {
        ...
        SineTable[i] = (uint16_t)(2047 + amplitude * sin(theta));
    }
}
//get the 9 zones
int GetZone(uint16_t adcValue){
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    {
        beepRequest = prevZone;  // request zone-number beeps
    }
}
